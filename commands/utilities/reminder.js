...
exports.data = new SlashCommandBuilder().setName('reminder').setDescription('Reminds a person.').addNumberOption(option => option.setName('date').setDescription('How many hours into the future?').setRequired(true)).addStringOption(option => option.setName('users').setDescription('Enter the @s of all the users to be mentioned').setRequired(true)).addStringOption(option => option.setName('message').setDescription('What message to send ').setRequired(false));
